...
[v1.1.0](https://github.com/chipkin/BACnetProfileExample-B-LD-CPP/releases/tag/v1.1.0)
(`metrics-windows.json` / `metrics-linux.json`):

<!-- METRICS -->
| Platform | Binary | Size | Start-up to `ready` | Stack commit |
|---|---|---|---|---|
| Windows x64 (windows-2022) | `BACnetExampleBLD.exe` | 3,253,760 bytes (~3.1 MiB) | 75 ms | `abd4cee1` |
| Linux x64 (ubuntu-latest) | `BACnetExampleBLD` | 45,128 bytes (~44 KiB) | 109 ms | `abd4cee1` |

v1.1.0 was published from a prebuilt STATIC-linked stack library rather than the
SOURCE-mode build documented above; the next release refreshes these numbers from
